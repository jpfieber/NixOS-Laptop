...
mount -o subvol=root,compress=zstd,noatime ${DEVICE}2 ${MOUNT_POINT}
mkdir -p ${MOUNT_POINT}/{boot,home,nix,.snapshots}
mount ${DEVICE}1 ${MOUNT_POINT}/boot
mount -o subvol=home,compress=zstd,noatime ${DEVICE}2 ${MOUNT_POINT}/home
mount -o subvol=nix,compress=zstd,noatime ${DEVICE}2 ${MOUNT_POINT}/nix
mount -o subvol=snapshots,compress=zstd,noatime ${DEVICE}2 ${MOUNT_POINT}/.snapshots
...
echo "5. Configure KDE manually (theme, icons, wallpaper)"
